...
module.exports = {
	...
	run: async (client, message, args) => {
		...
		if (!me.includes(message.author.id)) {
			...
			const { addbank } = require('../../functions.js');

			const balanceSchema = require('../../schemas/balanceSchema');
			const data = await balanceSchema.findOne({ UserID: userid });
			addbank(args[0], userid);
			data.save();
			message.channel.send(`I added ${args[0].toLocaleString()} coins to ${client.users.cache.get(userid).username}' bank!`);
		}
	} };
